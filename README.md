...
легко делиться конфигурацией через файл docker-compose.yml, экономить время на настройке окружения при каждом новом проекте, а также не засорять систему установкой множества сервисов напрямую.       
...
```
## Задание 6
## Настройка поочередности запуска, перезапуска и сети
